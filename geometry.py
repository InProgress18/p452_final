...
def indices_nearest_neighbors(coords):
    ...
    for i in range(len(coords)):
        for j in range(i + 1, len(coords)):
            if compute_distances(coords[i], coords[j]) < 2:  # Threshold distance for nearest neighbors
...
if __name__ == "__main__":
    ...
    for i in range(len(coords)):
        for j in range(i + 1, len(coords)):
            interaction_strength = compute_dipole_interaction(coords[i], coords[j], quantization_axis)
            print(f"Interaction strength between Spin {i} and Spin {j}: {interaction_strength:.6f}")
    ...
    for i, j in neighbors:
        ...
        if line_length > 0:
            ...
            orthogonal_vec = np.array([-line_unit_vec[1], line_unit_vec[0], 0])  # Rotate by 90 degrees in the xy-plane
            text_x = mid_x + orthogonal_vec[0] * 0.1  # Adjust the distance of the text from the line
            text_y = mid_y + orthogonal_vec[1] * 0.1
            ax.text(text_x, text_y, f"{interaction_strength:.2f}", fontsize=8, ha='center', va='center')
    plt.show()
